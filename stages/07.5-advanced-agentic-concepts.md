...
> 💡 這是一份 **進階概念地圖 + reading path**，不是完整教學。Stage 4 / 6 / 7 學完已能做能上線給人用的 agent（俗稱 production agent）；本 stage 幫你定位**業界還在討論哪些進階概念存在**、**每個概念解什麼問題**、**該先讀哪些 paper / blog**，避免你工作上踩到別人已踩過的坑。
...
Stage 4 教你挑 framework（agent 用哪個工具寫）、Stage 6 教 context engineering（怎麼塞東西給 agent 看）、Stage 7 教把 agent 「上線可穩定運作」——這 3 個 stage 加起來足以做 **70% 的 production-grade agent**（能給真人用、不會三天兩頭出包）。
...
- 是讓你**看別人寫的 agent 時、看得出它在做什麼**——舉例：別人的 agent 一出錯就「重試 N 次直到放棄」（=只有 retry）、跟它「出錯後先想一輪、修正方法再試」（即所謂 plan-act-reflect loop）、是完全不同等級的設計。能看出差別、你才知道該不該借用對方的做法
- 是讓你**知道每個概念該套到 agent 系統哪個部分、解哪一類問題**

## 🧭 概念地圖主軸：四層工作邊界（work boundary）

本 stage 用**工作邊界**作為整理進階 agentic workflow 的主軸：把 agent 系統拆成 4 個層級（**Types → Config → Repo → Service**、下面馬上展開）、然後問「agent 動的對象屬於哪一層？跨層越界會出什麼問題？」這不是把整章縮成一個模型、而是先給讀者一個定位座標、後面 12 個概念才能放到同一張圖上比較。
...
| 2 | **Contract-driven Hand-offs** | Types + Service | 上游 agent 承諾的 artifacts、下游 agent 必須驗證收到 | [Anthropic — Building Effective Agents](https://www.anthropic.com/engineering/building-effective-agents) Routing pattern |
...
| 8 | **Cost-aware Budget Gates** | Config（cost policy）| 超 \$ 預算自動停 / 升級審核（不只 token 上限）| [OpenAI Harness Engineering (2026-02)](https://openai.com/index/harness-engineering) |
