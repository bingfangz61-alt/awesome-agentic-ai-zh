...
> 💡 这是一份 **进阶概念地图 + reading path**，不是完整教学。Stage 4 / 6 / 7 学完后已经能做 production agent；本 stage 帮你定位**还有哪些 frontier 概念存在**、**每个概念解什么问题**、**该先读哪些 paper / blog**，避免你在真实工作里踩到别人已经踩过的坑。
...
但 frontier lab（Anthropic / OpenAI / Cognition / Microsoft）和学术界（Stanford / CMU / Princeton）在 2024-2026 推出了 12+ 个进阶概念，**有些你需要知道它们存在，即使现在用不到**。本 stage 不是要把它们包成单一理论、而是先给你一张可操作的概念地图：

- 不是要你全部学会
- 不是要你全部都用
- 是让你**遇到问题时知道该翻哪篇 paper / blog**
- 是让你**评估别人写的 agent 时，能说出「他在做 plan-act-reflect loop」、而不只是「他在 retry」**
- 是让你**知道每个概念大概落在哪一层、该用来解哪一类问题**

## 🧭 概念地图主轴：四层工作边界（work boundary）

本 stage 用**工作边界**作为整理进阶 agentic workflow 的主轴：agent 操作的对象属于 stack 的哪一层？跨层越界会出什么问题？这不是把整章缩成一个模型、而是先给读者一个定位坐标、后面的 12 个概念才能放到同一张图上比较。
...
- **越界没收手**（Cognition 的 Flappy Bird 案例）：用 multi-agent 拆解任务、其中一个 subagent 负责画绿色管道、另一个负责画云朵背景，结果合起来双方风格完全对不上——因为每个 subagent 只看到自己那块、没有对方在做什么的 context。Cognition 直白写道：「sub-agent 像一群过度自信的新人、根本不会在该问的时候问问题」。
  → 出处：[Cognition — Don't Build Multi-Agents (2025-06)](https://cognition.ai/blog/dont-build-multi-agents)

- **加料**（Anthropic Multi-Agent Research 的 speculative-leap 现象）：subagent 被指派「研究某主题」、它在报告里擅自加上「我推测 X 也可能成立、虽然我没验证」这类没人要的推论。Anthropic 在他们的 multi-agent 论文里专门讲为什么这种「主动补完」要 engineer 掉、不然 hallucination 会在 supervisor 没注意时偷渡进结果。
...
上图把 12 个概念按 **「位于 stack 的哪一层」**（横轴）和 **「解决什么类型的问题」**（纵轴）分群、让你一眼看出哪些概念适合一起学、哪些可以暂时跳过。其中 **Work Boundary（#1）跨所有层（discipline 级）、不是某个特定 stack 位置**。
...
下面 12 个概念用表格列出（# / 概念 / Stack 层 / 一句话定义 / 最佳读物）：

| # | 概念 | Stack 层 | 一句话定义 | 最佳读物 |
|---|---|---|---|---|
